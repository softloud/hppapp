...
tau <- function(nma_mod) {
  ...
  tibble::tibble(
    ...
    ggplot2::ggplot() +
    ggplot2::geom_density(ggplot2::aes(x = x), alpha = 0.6) +
    ggplot2::geom_vline(
    ...
    ggplot2::labs(
      ...
      subtitle = latex2exp::TeX(glue::glue("Mean $\\tau^2$ of {round(tau_vals$mean, 2)} across {nma_mod$network$studies %>% length()} studies")),
      y = "",
      x = "",
      caption = glue::glue(
